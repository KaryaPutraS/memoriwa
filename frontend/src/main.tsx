...
function DocRow({doc,sel,toggle,analyze,del,onEdit,folders,onIdentify,onShare}:{doc:Doc;sel:string[];toggle:(id:string)=>void;analyze:()=>void;del?:()=>void;onEdit?:(id:string,patch:any)=>void;folders?:string[];onIdentify?:(id:string)=>void;onShare?:(type:string,id:string)=>void}) {
  ...
  return <div className="dw">
    ...
    {o&&<div className="dp"><div className="dg"><div className="dp-info"><b>{doc.filename}</b>
      ...
      {(isPpt||isDocx||isXlsx)&&<div className="pm pm-office" style={{width:'100%'}}><iframe src={`/api/files/${doc.id}/view?token=${encodeURIComponent(localStorage.getItem('token')||'')}`} style={{width:'100%',height:380,border:'1px solid #334155',borderRadius:10,background:'#0f172a'}} title={doc.filename}/></div>}
      ...
      <div className="pa">
        ...
        <a className="btn sm" href={`/api/files/${doc.id}/view?token=${encodeURIComponent(localStorage.getItem('token')||'')}`} target="_blank" rel="noopener"><Share2 size={12}/> Open Preview</a>
        <a className="btn sm" href={pv||`/api/files/${doc.id}/raw?token=${encodeURIComponent(localStorage.getItem('token')||'')}`} target="_blank" rel="noopener" download={doc.filename}><FolderInput size={12}/> Download File</a>
      </div></div>
    }
